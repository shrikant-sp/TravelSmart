...
def save_to_excel(name, user_id, hashed_password):
    """
    Saves user credentials to an Excel file by reading the old history,
    adding the new user, and saving the updated list.
    """
    new_data = {
        'Username': [name],
        'User ID (Email)': [user_id],
        'Password (Code Language)': [hashed_password],
    ...
    if os.path.exists(EXCEL_FILE):
        try:
            # 1. Read the existing Excel file to get previous users
            existing_df = pd.read_excel(EXCEL_FILE)
            
            # 2. Combine (Concatenate) the old data with the new data
            updated_df = pd.concat([existing_df, new_df], ignore_index=True)
            
            # 3. Save the combined data back to Excel (Overwriting the file with new list)
            updated_df.to_excel(EXCEL_FILE, index=False)
            print(f"User {name} added to {EXCEL_FILE}")
            
        except PermissionError:
            print("ERROR: Could not save! Please CLOSE the Excel file and try again.")
        except Exception as e:
            print(f"An error occurred: {e}")
    else:
        # If file doesn't exist, create it with the first user
        new_df.to_excel(EXCEL_FILE, index=False)
        print(f"Created {EXCEL_FILE} with first user")
...
def signup():
    ...
    # --- [MODIFIED] SAVE TO EXCEL ---
    try:
        # We save the data when they SIGN UP
        save_to_excel(name, email, hashed_password)
    except Exception as e:
...
def auth_callback(provider):
    ...
    try:
        ...
        if email not in users:
            ...
            hashed_pass = generate_password_hash(str(uuid.uuid4()))
            users[email] = {
                'name': name,
                'email': email,
                'password': hashed_pass, # Random secure password
                'is_admin': False,
                'reward_points': 0,
                'provider': provider, # Track which provider they used
                'created_at': datetime.now().strftime('%Y-%m-%d %H:%M')
            }
            # Also save social logins to Excel
            save_to_excel(name, email, hashed_pass)
